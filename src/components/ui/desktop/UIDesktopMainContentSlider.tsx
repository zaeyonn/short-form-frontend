...
const UIDesktopMainContentSlider = (props: Props) => {
  ...
  const handleMouseMove = (
    e: React.MouseEvent<HTMLElement> | React.UIEvent<HTMLDivElement> | any
  ) => {
    ...
    if (listRef.current) {
      ...
      listRef.current.scrollLeft =
        startScrollRef.current - scrollDistanceRef.current;
    }
  };
    ...
    if (draggingRef.current && listRef.current) {
      ...
      itemPositionRef.current =
        scrollDistanceRef.current < 0
          ? Math.floor(currentScroll / ITEM_WIDTH) + 1
          : Math.floor(currentScroll / ITEM_WIDTH);
        ...
        {extendedList.map((item: Series, index: number) => {
          return (
            <div
              ...
              key={`${item.id}-${index}`}
              onMouseUp={() => handleSeriesMouseUp(item)}
            >
